...
type SlotReq = { id: string; name: string; credits: number; spins: number; image: string };
...
function PackCardList({ t }: { t: Dict }) {
  ...
  const open = (pack: CardPack, idx: number) =>
    openSlot({ id: pack.id, name: t.storePackNames[idx] ?? pack.name, credits: pack.credits, spins: pack.spins, image: pack.image });
  return (

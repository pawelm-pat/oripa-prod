    ...
    >
      {spinKey === 0 ? (
        <div
          ...
          style={{ backgroundImage: "repeating-linear-gradient(45deg,#242430 0 8px,#1c1c26 8px 16px)" }}
        />
      ) : (
        <div ref={stripRef}>
          {strip.map((s, i) => (
      ...
      <div className="absolute inset-0 z-[70] flex flex-col bg-[#0d0d12] text-white" style={{ animation: "slotIn .25s ease" }}>
        <div className="flex-1 overflow-y-auto px-4 pt-5 pb-4">
          <h2 className="text-[18px] font-extrabold leading-tight">
            ...
            <span className="text-white/90"> {lang === "ja" ? "— あなたのストック" : "— your stack"}</span>
          </h2>
          <p className="mt-1 text-[11px] font-medium text-white/50">{L.summary(credits, spins, won.length)}</p>
          {won.length === 0 ? (
            <p className="mt-10 text-center text-[13px] text-white/50">{L.noCards}</p>
          ) : (
            <div className="mt-4 grid grid-cols-3 gap-3">
              {won.map((c) => (
                <div key={c.id} className="flex flex-col items-center">
                  <img src={RARITY_IMG[c.rarity]} alt="" className="w-full rounded-lg object-cover" style={{ aspectRatio: "5/7", boxShadow: "0 2px 8px rgba(0,0,0,0.4)" }} />
        ...
        <div className="flex shrink-0 gap-3 border-t border-white/10 px-4 py-3">
          <button onClick={onClose} className="flex-1 rounded-xl bg-white/10 py-3 text-[14px] font-bold text-white active:scale-[0.99]">{L.backToShop}</button>
  ...
  return (
    <div className="absolute inset-0 z-[70] flex flex-col bg-[#0d0d12] text-white" style={{ animation: "slotIn .25s ease" }}>
      ...
      <div className="shrink-0 px-4 pt-4">
        <div className="flex items-center justify-between">
          <p className="text-[13px] font-bold">
            <span style={{ color: "#D10005" }}>{packName}</span>
            <span className="text-white/60"> · {L.spinsLeft(spinsLeft)}</span>
          </p>
          <div className="flex items-center gap-2">
            <p className="text-[13px] font-extrabold">{payoutPct}% <span className="text-[10px] font-medium text-white/50">{L.toPayout}</span></p>
            <div className="relative flex h-7 w-7 items-center justify-center rounded-lg bg-[#17171d]" style={{ boxShadow: "inset 0 1px 4px rgba(0,0,0,0.6)" }}>
        ...
        <p className="mt-2 text-[10px] leading-snug text-white/40">{L.stacked(won.length)}</p>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-center px-4">
        ...
        <p className="mt-3 text-center text-[10px] font-medium text-white/40">{L.winsDrop}</p>
      </div>
      ...
      <div className="shrink-0 px-4 pb-4">
        ...
        <div className="mt-3 flex items-center justify-between text-[11px] font-semibold text-white/60">
          <button onClick={onClose} className="flex items-center gap-1 active:text-white">
          ...
          <button onClick={() => setQuick((q) => !q)} className="flex items-center gap-1" style={{ color: quick ? "#f5a623" : undefined }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L3 14h7l-1 8 10-12h-7z" /></svg>
            {L.quickSpin}{quick ? " ✓" : ""}
          </button>
          <button onClick={fastForward} className="flex items-center gap-1 active:text-white">
            {L.fastForward}
      ...
      {/* Card reveal */}
      {reveal && (
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/70 px-6" onClick={dismissReveal}>
          <div className="w-full max-w-[300px] rounded-2xl bg-[#17171d] p-5 text-center" style={{ animation: "revealPop .3s cubic-bezier(.2,.9,.3,1.2)" }} onClick={(e) => e.stopPropagation()}>
            {reveal.big && <p className="mb-2 text-[15px] font-black tracking-wide" style={{ color: "#f5a623" }}>{L.bigWin}</p>}
            ...
            <p className="mt-3 text-[16px] font-extrabold text-white">{lang === "ja" ? reveal.cards[0].nameJa : reveal.cards[0].name}{reveal.cards.length > 1 ? ` ×${reveal.cards.length}` : ""}</p>

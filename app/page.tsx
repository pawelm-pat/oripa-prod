...
import { LangToggle, PhoneApp, SlotVersionToggle, UpdatePrompt, VersionBadge } from "./components/oripa";
...
export default function Page() {
  ...
  // Sandbox experiment: which slot-game skin to use (Real slot disabled for now).
  const [slotVersion, setSlotVersion] = useState<1 | 2>(1);
  return (
    <main className="flex min-h-[100svh] w-full flex-col items-center justify-center bg-[linear-gradient(180deg,#16171c_0%,#0f1014_100%)]">
      ...
      <div className="relative hidden sm:block py-8">
        <div className="absolute right-full top-3 mr-4 flex w-max flex-col items-end gap-3">
          <LangToggle lang={lang} setLang={setLang} />
          <SlotVersionToggle version={slotVersion} setVersion={setSlotVersion} />
        </div>
        <div className="rounded-[2.6rem] border border-white/12 bg-[#1b1c22] p-3 shadow-[0_35px_90px_rgba(0,0,0,0.55)]">
          <div className="rounded-[2.1rem] border border-white/8 bg-black p-2">
            ...
            <div className="relative h-[812px] w-[390px] overflow-hidden rounded-[1.7rem] bg-[#eef0f3]">
              <PhoneApp lang={lang} slotVersion={slotVersion} noHistory={false} onScreenChange={setScreen} />
            </div>
          </div>
        </div>
      </div>
